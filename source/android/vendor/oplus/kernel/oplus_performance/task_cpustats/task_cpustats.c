...
static int cpu_irqinfo_show (struct seq_file *m, void *v)
{
...
static int irqinfo_open(struct inode *inode, struct file *file)
{
	return single_open(file, cpu_irqinfo_show, NULL);
}
...
static struct list_head pid_trace_list;
static struct list_head tid_trace_list;
int sysctl_cpu_tid_jank_info_enable = false;

enum trace_list_type {
	TID_LIST = 0,
	PID_LIST
};

static void tid_jank_info_show(struct seq_file *m)
{
	return;
}

static void tgid_jank_info_show(struct seq_file *m)
{
	return;
}

static int cpu_jank_info_show (struct seq_file *m, void *v)
{
	if (list_empty(&pid_trace_list))
		return 0;
	else
		tgid_jank_info_show(m);

	if (list_empty(&tid_trace_list) || !sysctl_cpu_tid_jank_info_enable)
			return 0;
	else
		tid_jank_info_show(m);

	return 0;
}

static int cpu_jank_info_open(struct inode *inode, struct file *file)
{
	return single_open(file, cpu_jank_info_show, NULL);
}

static ssize_t  cpu_jank_info_write(struct file *file, const char __user *user_buf,
			 size_t count, loff_t *ppos)
{
	return 0;
}

static const struct file_operations cpu_jank_info_proc_fops = {
	.open		= cpu_jank_info_open,
	.read		= seq_read,
	.write		= cpu_jank_info_write,
	.llseek		= seq_lseek,
	.release	= single_release,
};

void trace_task_del(struct task_struct *p)
{
	return;
}
...
static int __init proc_task_cpustat_init(void)
{
	...
	proc_create("cpu_jank_info", S_IRUGO | S_IWUGO, NULL, &cpu_jank_info_proc_fops);
	INIT_LIST_HEAD(&pid_trace_list);
	INIT_LIST_HEAD(&tid_trace_list);
	return 0;
}
